...
// for crc --> take a look at https://github.com/lammertb/libcrc
// and https://www.lammertbies.nl/comm/info/crc-calculation
...
/*  Function to find valid HDLC frame from incoming data - 
    A byte at a time*/
void Arduhdlc::frameDecode_char(uint8_t data)
{
    /* FRAME FLAG */
    if(data == FRAME_BOUNDARY_OCTET)
    {
        ...
        else if( (this->frame_position >= 2)  ) // checksum here
                     // (msb << 8 ) | (lsb & 0xff)
        {
...
void Arduhdlc::frameEncode(const uint8_t *framebuffer, uint8_t frame_length)
{
    uint8_t data ;
